...
function transformMeetsData(meetsData) {
  return meetsData
    .filter(meet => {
      // Skip meets that contain "ADAPTIVE" in their name
      if (meet.name.toUpperCase().includes('ADAPTIVE')) {
        console.log(`Skipping ADAPTIVE meet: "${meet.name}"`);
        return false;
      }
      return true;
    })
...
async function sendSlackNotification(meetsAddedCount, meetNames = []) {
  ...
  // Create the message
  let message = `${meetsAddedCount} Virus Series meets added to Supabase`;
